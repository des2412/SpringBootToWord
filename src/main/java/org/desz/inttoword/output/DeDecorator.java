...
public class DeDecorator implements IWordDecorator {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.desz.inttoword.output.IWordDecorator#pluraliseUnitRule(java.lang.
	 * String)
	 */
	@Override
	public WordResult pluraliseUnitRule() {
		...
		if (Objects.nonNull(wordResult)) {
			if (Objects.nonNull(wordResult.getBill())) {
				final String bill = StringUtils.normalizeSpace(wordResult.getBill());
				final String num = bill.split(SPC.val())[0];
				if (!num.matches(DePair.ONE.getWord()))
					builder.withBill(StringUtils.normalizeSpace(bill + "n"));

				else
					builder.withBill(StringUtils.normalizeSpace(wordResult.getBill()));
			}
			...
			if (Objects.nonNull(wordResult.getMill())) {
				...
				if (!num.matches(DePair.ONE.getWord()))
					builder.withMill(StringUtils.normalizeSpace(wordResult.getMill()) + "en");

				else
					builder.withMill(StringUtils.normalizeSpace(wordResult.getMill()));
			}
	...
	public WordResult pluraliseOneRule(int val) {
		...
		if (Objects.nonNull(wordResult.getHund())) {
			if (BigInteger.valueOf(val).mod(BigInteger.valueOf(100)).equals(BigInteger.ONE))
	...
	public WordResult restructureHundrethRule() {
		...
		if (Objects.nonNull(wordResult.getMill())) {
			...
			if (arr.length > 2) {
				String s = arr[1] + DeUnit.AND.val() + arr[0] + SPC.val() + arr[2] + SPC.val();
				res.withMill(s);
