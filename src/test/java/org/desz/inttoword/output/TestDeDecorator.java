...
public class TestDeDecorator {
	...
	public void testPluraliseOneRule() {
		...
		String input = StringUtils.normalizeSpace(DePair.ONE.getWord() + SPC.val() + deLangMap.getMilln());
		...
		input = StringUtils.normalizeSpace(DePair.TWO.getWord() + SPC.val() + deLangMap.getMilln());
		builder = new WordResult.Builder();
		builder.withMill(input);
		deDecorator = new DeDecorator(builder.build());
		...
		input = StringUtils.normalizeSpace(DePair.TWO.getWord() + SPC.val() + deLangMap.getBilln());
		builder = new WordResult.Builder();
	...
	public void testReplaceSpaceWithEmptyRule() {
		...
		assertFalse("Empty Space in thousandth unit shouldn't be present", res.getThou().trim().contains(SPC.val()));
	}
